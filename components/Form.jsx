...
const Form = () => {
  ...
  return (
    <>
        <h1 className="text-3xl text-blue-600 mb-2">Add New Routine</h1>
        <form onSubmit={submit} className='bg-gray-100 px-8 py-4 rounded'>
            ...
            <div className="flex md:flex-row flex-col items-center py-8 px-4 flex-wrap gap-y-3">
                {/* Code block starts */}
            ...
            <div className="flex md:flex-row flex-col items-center py-8 px-4 flex-wrap gap-y-3 max-h-96 overflow-y-auto">
                {activeEx.map((exer,i)=>{
                    return <div key={i} className="flex flex-row md:mr-16 bg-gray-300 px-5 py-2 rounded-lg items-center w-full justify-between">
                    <div>
                    ...
                    <div className='flex flex-row flex-wrap gap-4 ml-4 p-4 capitalize'>
                    ...
                    <div>
                        <button disabled={i===0} type='button' className='font-bold text-xl disabled:text-gray-100' onClick={(e)=>{moveUp(e,i)}}><ArrowDropUpIcon/></button>
                        <button disabled={i===(activeEx.length-1)} type='button' className='font-bold text-xl disabled:text-gray-100' onClick={(e)=>{moveDown(e,i)}}><ArrowDropDownIcon/></button>
                        <button disabled={activeEx.length===1} type='button' className='text-red-600 font-bold text-xl disabled:text-gray-100' onClick={(e)=>delExs(e,i)}><DeleteIcon/></button>
                    </div>
                </div>
                })}

            </div>
            <button type="button" className="bg-indigo-700 transition duration-150 ease-in-out hover:bg-indigo-600 rounded text-white px-3 py-1 text-md disabled:bg-slate-400 mt-2" disabled={activeEx.length===exercisesState.length} onClick={addExs}>Add More Exercises</button>
